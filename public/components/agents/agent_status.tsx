...
import { convertCamelCase, translate } from '../common/util/common/string';

const getAgentStatusTrans = (status) => translate(`common.status.${convertCamelCase(agentStatusLabelByAgentStatus(status))}`)
...
export const AgentStatus = ({ status, children = null, labelProps = {}, style = {} }) => (
	<span className="euiFlexGroup euiFlexGroup--gutterExtraSmall euiFlexGroup--alignItemsCenter euiFlexGroup--directionRow" style={style}>
		<EuiToolTip position="top" content={getAgentStatusTrans(status).toLowerCase()}>
